...
public class games_GET_gamecardProps extends TestBase {
	...
	public void gamecardProps() throws JsonProcessingException {
		if (test == null) {
			test = extent.createTest("Grizzly Admin Should be able to get all games list");
			extentTest.set(test);
			flag = true;
		}
		extentTest.get().log(Status.INFO, "Grizzly Admin Should be able to get games card props");
		response = (Response) requests.getUsers("/api/users/" + userId + "/games/" + gameId + "/cards",
				extentTest.get());
		System.out.println(response.body().prettyPeek());

		log = extentTest.get().createNode("To Verify Status Code :: games card props");
